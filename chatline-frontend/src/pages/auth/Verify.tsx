...
// Handles the magic-link flow: auto verify via query params then route the user back to login.
function Verify() {
  const [params, setParams] = useSearchParams();
  const navigate = useNavigate();
  const dispatch = useDispatch<AppDispatch>();
  ...
  const callVerification = useCallback(async () => {
    console.log(`Called ${data.id} ${data.email} ${data.otp}`);
    if (data.id && data.email && data.otp) {
      // Dispatching here triggers the otp verification thunk once the URL carries all three parameters.
      dispatch(verifyEmail(data));
    }
  }, [data, dispatch]);

  useLayoutEffect(() => {
    // Run verification as soon as the component paints to avoid a visible flash of the loading UI.
    callVerification();
  }, [callVerification]);
  ...
  useEffect(() => {
    const call = async () => {
      if (!authSlice.loading && authSlice.success) {
        ...
        // Redirect back to login so the user can sign in with their new verified status.
        navigate("/login");
      }
    };
    call();
