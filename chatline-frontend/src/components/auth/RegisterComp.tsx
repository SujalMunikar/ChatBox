...
const RegisterComp: React.FC = () => {
  ...
  // Enforce basic constraints so the API receives clean registration data.
  ...
  // Pre-fill only the email for quicker QA; leave the name blank so testers supply it manually.
  useEffect(() => {
    setValue("name", "");
  ...
  // When registration succeeds, clear the form and send the user back to login.
  useEffect(() => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    // Kick off server-side account creation.
    dispatch(
      registerApi({
        email: data.email,
  ...
  const onSubmitError = (error: Record<string, unknown>) => {
    // Surface validation errors in the console for debugging.
