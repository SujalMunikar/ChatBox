...
// Reuse email sizing helper so long addresses wrap nicely in both layouts.
...
// Dedicated panel to review and accept or decline requests others sent to you.
export default function IncomingRequests() {
  const dispatch = useAppDispatch();
  const friends = useAppSelector((state) => state.friends);
  // Stash the user's preferred default layout under a unique key.
  const [viewMode, setViewMode] = usePersistedViewMode("friends:view:incoming", "card");

  const refresh = () => {
    // Re-sync after accept/decline to reflect latest server state.
  ...
  const handleAccept = (id: string) => {
    dispatch(acceptFriendRequest({ id, onSuccess: refresh })); // When the server confirms, refresh to hydrate lists.
  };
              ...
              {friends?.incomingRequests.map((item: any) => (
                ...
                >
                  ...
                  <div className="p-6 pb-2 text-center">
                    ...
                    <p className={getEmailClassName(item?.sender?.email, "card")}>
                      {item?.sender?.email}
                    </p>
                  </div>
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className={getEmailClassName(item?.sender?.email, "list")}>{item?.sender?.email}</div>
                  </div>
                </div>
