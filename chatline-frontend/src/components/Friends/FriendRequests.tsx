...
// Shows a raw view of every outgoing and incoming request; mostly for admin/debug usage.
export default function FriendRequests() {
  const dispatch = useAppDispatch();
  const friends = useAppSelector((state) => state.friends);
  // Shared helper keeps both lists aligned after accepting a request.
  const fetchIncomingAndOutgoingRequests = () => {
    // Pull both hands of the pending requests list so the panel stays in sync after actions.
    dispatch(getOutgoinFriendRequests());
    dispatch(getIncomingFriendRequests());
  };
  // Mirrors componentDidMount to prefill both tables on first render.
  useEffect(() => {
