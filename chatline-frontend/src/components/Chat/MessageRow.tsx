...
function MessageRow(props: MessageRowType) {
  const { isSent, message, data } = props;
  const { authState } = useAuth();
  const conversationUser = useAppSelector((state) => state.user.currConversationUser);
  ...
  const renderMessage = () => {
    try {
      ...
      try {
        ...
        // Corrupted payloads shouldn't break the UI; log and fall back to the raw string.
  ...
  const formatTime = (dateString: string) => {
    try {
      ...
      // Keep a deterministic fallback so the bubble layout stays the same if parsing fails.
      return "2:21 PM"; // fallback
    }
  };

  const incomingName = conversationUser?.name || data?.senderName || data?.sender?.name;
  const incomingImage = conversationUser?.image || data?.sender?.image;
  ...
  return (
    ...
    >
      {!isSent && (
        <div className="left pb-5">
          <Avatar name={incomingName} image={incomingImage} size={sizeList.small} />
        </div>
      )}
      <div
        className={`right flex flex-col ${
