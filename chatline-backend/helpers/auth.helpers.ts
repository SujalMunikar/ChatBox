...
export const generateOTP = (): string => {
  // Produce a six-digit numeric code as a string for email verification.
  const otp = Math.floor(100000 + Math.random() * 900000).toString();
  return otp;
};

export const generateToken = async (payload: any) => {
  // Sign a JWT containing the provided payload using the shared secret.
  return jwt.sign(
    {
      ...payload,
    },
...
export const deleteCookieForToken = async (res: Response) => {
  // Overwrite the cookie with an empty value so the browser drops it immediately.
  return res.cookie("jwt", "", { maxAge: 0 });
};

export const verifyToken = async (token: string) => {
  // Recreate the payload from the token and throw if the signature/expiry is invalid.
...
export const sendOtpEmail = async (body: any, otp: string) => {
  try {
    ...
    if (!otp) {
      // Guard clause so we never send an email missing the actual token.
      // TODO
    }
    const htmlToSend = `
    <h1>OTP : ${otp}</h1>
    ...
    await transporter.sendMail(
      {
        from: '"Chatline" <astromoon7777@gmail.com>', // sender address
